...
- **RQ1:** 2016–2024년 뉴스 데이터를 기반으로 HBM, DDR, NAND 등 반도체 핵심 기술 토픽은 시간에 따라 어떠한 진화 패턴을 보이는가?
  - **답변:** HBM은 2023년을 기점으로 급격한 상승세를 보이며(선형 회귀 계수 SK하이닉스 3.775), AI 산업 성장과 강한 동반 상승 패턴을 나타냈다. DDR은 완만한 증가세를 유지한 반면, NAND는 정체 또는 감소 패턴을 보였다. BERTopic 분석을 통해 'DRAM' 중심 토픽이 시간에 따라 'HBM'과 'AI 메모리'로 분화되는 진화 경로를 확인하였다.

- **RQ2:** 동일 기간 동안 삼성전자와 SK하이닉스의 기술·전략 포지셔닝(집중형 vs 분산형)은 어떻게 다른가?
  - **답변:** 의미 연결망 분석(SNA) 결과, SK하이닉스는 'HBM-AI-엔비디아' 키워드를 중심으로 한 **집중형(Focused) 전략**을 보였으며 네트워크 중심성이 매우 높게 나타났다. 반면 삼성전자는 '메모리-파운드리-시스템LSI-스마트폰' 등 다양한 키워드가 네트워크 전반에 분산된 **분산형(Diversified) 전략**을 보였다. 이는 두 기업의 사업 포트폴리오 및 경쟁 전략 차이를 명확히 반영한다.

- **RQ3:** BERTopic–SNA–LLM으로 구성된 하이브리드 분석이 기존의 단순 빈도·정적 토픽 모델링 대비 어떤 추가 인사이트(약한 신호, 맥락 해석 등)를 제공하는가?
  - **답변:** 단순 빈도 분석은 'HBM 언급량 증가'라는 표면적 사실만을 제공한다. 반면 제안된 하이브리드 프레임워크는 ① **BERTopic**을 통해 토픽의 분화 과정(DRAM→HBM)을 시계열로 추적하고, ② **SNA**로 기업별 전략 구조 차이를 시각화하며, ③ **LLM**을 통해 네트워크 주변부에서 '하이브리드 본딩', 'CXL' 등 **약한 신호(Weak Signal)**를 탐지하고, HBM 급증의 원인을 "생성형 AI 인프라로서의 재정의"라는 **맥락적 해석**으로 제공하였다. 이는 정량 분석과 정성 추론의 시너지를 입증한다.
